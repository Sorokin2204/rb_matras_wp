...
function ddtsug_woo_fields_start(){
	...
	if(get_option('ddtsug_woo_need_address')!=true) {
		...
		echo '<input id="address" name="address" type="text" size="57" placeholder="Введите Ваш адрес в произвольной форме"/>';
	}
	...
	if(get_option('ddtsug_woo_need_spoiler')==true) {
		echo <<<EOT
<div class="ddtsugspoiler ddtsugclosed">
...
add_action('woocommerce_before_checkout_billing_form', 'ddtsug_woo_fields_start');
